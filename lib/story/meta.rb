module Story
  ...
  module Errors
    ...
    def raise_unsupported_db_adapter file, config
      "Unsupported database adapter <b>#{config["adapter"]}</b> in database config file (#{print_root})."
    end

    def raise_no_db_adapter_specified file, config
      "No database adapter specified in database config file (#{print_root})."
    end

    def raise_no_database_specified file, config
      "No database file or data specified in database config file (#{print_root})."
    end

    def raise_no_db_adapter_and_database_specified file, config
      "No database file and data specified in database config file (#{print_root})."
    end
